...
const FilterFoodItems = ({dispatch, state}) => {
  const itemListLength = state.items.filter((item)=> item.category === "Food").length
  return itemListLength === 0 ? "No Items Available" : state.items.filter((item)=> item.category === "Food").map((item, index, id)=>(
    <>
