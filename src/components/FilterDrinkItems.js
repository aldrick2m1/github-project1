...
const FilterDrinkItems = ({dispatch, state}) => {
  const itemListLength = state.items.filter((item)=> item.category === "Drink").length
  return itemListLength === 0 ? "No Items Available" : state.items.filter((item)=> item.category === "Drink").map((item, index, id)=>(
    <>
...
export default FilterDrinkItems